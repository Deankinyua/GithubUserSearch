defmodule DevfinderWeb.TitleLive.Component do
  ...
  def render(assigns) do
    ...
    <div class="flex justify-between items-center w-full sm:w-9/12 md:w-11/12">
      ...
      <.link phx-click={JS.push("dark-mode", value: %{dark: @is_dark})}>
        <section class="flex items-center gap-4 mono-semibold group">
          <div class="text-sm tracking-wider text-[#697C9A] group-hover:text-[#2B3442] transition-ease-in-out duration-300 dark:text-[#FFFFFF] dark:group-hover:text-[#4B6A9B]">
            {@theme}
          </div>
            ...
            <div class={@moon}>
              <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg">
                <path
                  ...
                  class="fill-[#697C9A] group-hover:fill-[#2b3442] transition-ease-in-out duration-300"
                  fill-rule="nonzero"
                />
              </svg>
            </div>
              ...
              <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg">
                <g fill-rule="nonzero">
                  <path
                    ...
                    class="fill-[#FFFFFF] group-hover:fill-[#4B6A9B] transition-ease-in-out duration-300"
                  />
                </g>
              </svg>
            </div>
  ...
  def update(assigns, socket) do
    is_dark = assigns.is_dark

    socket = form_theme_icon(socket, is_dark)
